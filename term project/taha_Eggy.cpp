...
int delay=0,loadFlag=1,loadCmpltd=0,theatricalCmpltd=0,startPressed=0,startMenuCmpltd=0,newGamePressed=0,loadGamePressed=0,
	exitPressed=0, hofPressed=0, creditPressed=0, instructionPressed=0, gameStart=0,egg1=0,egg2=0,egg3=0,egg4=0,egg5=0,ex1=0,ex2=0,ex3=0,ex4=0,ex5=0,ey=410,dwn1=0,dwn2=0,dwn3=0,dwn4=0,dwn5=0,
	hx=0,dx=5,nx=192,score=0,golden=0,green=0,bomb=0,level,gld,grn,red,brkex;

int pauseGamePressed=0, pauseResumePressed=0, pauseSavePressed=0, pauseMenuPressed=0, saveGame=0, slotSelect=0, 
	slot1=0, slot2=0, slot3=0, slotSelectLG=0, LGslot1=0, LGslot2=0, LGslot3=0, OKpressed=0,cancelPressed=0,
	gameOver=0, congratsShow=0, nameInput=0, textboxVisit=0, hofShow=0, thanksShow=0, creditShow=0, instructionShow=0;

int textbox=0,len=0,warning=0,clicked=0,eggBreak=0;

char Score[100];
char saveFile[15]; //saveFile1[100];
char output[100];
...
void iNewInitialize()
{
	...
	life=10,hen2Flag=1,hen2mFlag=1;
...
//    reverse(str, str+i);
//
//    if(type == 1) strcpy(output, str);
//    else strcat(output, str);
//}
...
//	save[0].score = score;
//	save[0].life = life;
//	fprintf(fsave, "%s %d %d", saveFile, save[0].score, save[0].life);
//	fclose(fsave);
//}
//
//void loadFileFromPC()
...
//
//	fload = fopen("saved game.txt", "r");
//	fscanf(fload, "%s %d %d", save[0].savedFile, &save[0].score, &save[0].life);
//	score = save[0].score;
//	life = save[0].life;
//	fclose(fload);
//}
...
void iLoad()
{
	...
	if (delay==10) {loadFlag=2; delay=0;}
	...
	if (delay==10) {loadFlag=3; delay=0;}
	...
	if(loadFlag==3)
	{
		...
		if(delay==10) {loadCmpltd=1; delay=0;}
	}
...
void iEggFall()
{
		if(egg1==1)
		{
			...
			else if (dwn1>=380) {brkex= ex1; eggBreak=1; life--;}
		...
		if (egg2==1)
		{
			...
			else if (dwn2>=380) {brkex= ex2; eggBreak=1; life--;}
...
void iGame()
{
	...
	if(gameStart==1)
	{
		...
		if (dontDraw!=1)
		{
			...
			if(eggBreak==1)
			{
				iShowBMP( brkex - 30, 5, "egg break.bmp");
...
void gamePaused()
{
	...
	if(pauseResumePressed==1 && pauseSavePressed==0 && pauseMenuPressed==0)
	{
		iShowBMP(151,433,"resume 2.bmp");

		delay++;
		if(delay==10) {pauseGamePressed=0; pauseResumePressed=0; gameStart=1; delay=0; iResumeTimer(0);}
	}
	if(pauseResumePressed==0 && pauseSavePressed==1 && pauseMenuPressed==0)
	{
	...
	if(pauseResumePressed==0 && pauseSavePressed==0 && pauseMenuPressed==1)
	{
...
void saveGameWindow()
{
	...
	if(warning == 1)
	{
		iSetColor(192,255,255);
		iText(95,206,"Sorry mate! So SPACE allowed!",GLUT_BITMAP_TIMES_ROMAN_24);
...
void slotSelectWindowLG()
{
	...
	if (LGslot1==1 || LGslot2==1 || LGslot3==1) 
	{
		newInit=1;
		if (level==2) levelChange2=0;
		if (level==3) levelChange3=0;
...
void nameForFame()
{
	...
	{
		iSetColor(0,0,94);
		iText(176,508,"Sorry mate! So SPACE allowed!",GLUT_BITMAP_TIMES_ROMAN_24);
		iText(188,488,"You can use underscore(_) instead.",GLUT_BITMAP_HELVETICA_18);
	}

	if(textbox == 0 && textboxVisit == 1)
	{
		sort(hof, hof+11, comp);
		StructureToFileHOF();
...
void iMouse(int button, int state, int mx, int my)
{
	if(button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
	{
		...
		{
			if((mx >= 136 && mx <= 362) && (my >= 386 && my <= 445)) LGslot1=1;
			if((mx >= 136 && mx <= 362) && (my >= 316 && my <= 375)) LGslot2=1;
			if((mx >= 136 && mx <= 362) && (my >= 247 && my <= 306)) LGslot3=1;
		}
...
void iKeyboard(unsigned char key)
{
	if (textbox == 1) 
	{
		...
		else if(key == 32)
		{
			...
			/*{
				iSetColor(192,255,255);
				iText(80,206,"Sorry mate! So SPACE allowed!",GLUT_BITMAP_TIMES_ROMAN_24);
				iText(70,180,"You can use underscore(_) instead.",GLUT_BITMAP_TIMES_ROMAN_24);
			}
			...
			else if(nameInput == 1)
			{
				iSetColor(217,0,0);
				iText(176,508,"Sorry mate! So SPACE allowed!",GLUT_BITMAP_TIMES_ROMAN_24);
				iText(196,488,"You can use underscore(_) instead.",GLUT_BITMAP_HELVETICA_18);
			}*/
		}
