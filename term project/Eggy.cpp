...
int delay=0, loadFlag=1, loadCmpltd=0, theatricalCmpltd=0, startPressed=0, startMenuCmpltd=0,
	newGamePressed=0, loadGamePressed=0, exitPressed=0, hofPressed=0, creditPressed=0, instructionPressed=0,
	gameStart=0, egg1=0, egg2=0, egg3=0, egg4=0, egg5=0,
...
int  newInit=0, textbox=0, len=0, warning=0, clicked=0, flagEgg=0, fileWarning = 0;

int breakdelay, pausedelay, thanksdelay, gameoverdelay, canceldelay, okdelay, congratsdelay, newgamedelay, loadgamedelay;

int levelDetect=0, henSpeed=0, range=5, fall=5, hen1Mirror=0, hen2Mirror=0, hen3Mirror=0, levelChange2=1, 
	levelChange3=1, dontDraw=0, life=10, maxlife;
...
void iEggFall()
{
		...
		{
			dwn1=dwn1+fall;
			if(nx>=ex1-84 && nx<=ex1-26 && dwn1==380) {score=score+10;}
		...
		{
			dwn2=dwn2+fall;
			if(nx>=ex2-84 && nx<=ex2-26 && dwn2==380) {score=score+10;}
		...
		{
			dwn3=dwn3+fall;
			if(nx>=ex3-84 && nx<=ex3-26 && dwn3==380) 
			{
				if (life<maxlife) life++;
				else if(life==maxlife) score=score+30;
			}
		...
		{
			dwn4=dwn4+fall;
			if(nx>=ex4-84 && nx<=ex4-26 && dwn4==380) {score=score+25;}
		...
		{
			dwn5=dwn5+fall;
			if(nx>=ex5-84 && nx<=ex5-26 && dwn5==380) {score=score-15;}
...
void iGame()
{
	...
	if(gameStart==1)
	{
		iResumeTimer(0);
		//if (newInit==0 ) { iNewInitialize(); newInit=1;}
		if (levelDetect<500) level=1;
		else if(levelDetect>=500 && levelDetect<1000) level=2;
		else level=3;
		...
		else if(level==2)
		{
			...
			if (levelChange2==1)
			{
				...
				if (delay==100) 
				{
					...
					maxlife=13; gld=15; grn=10; red=6; flagEgg=0;
		...
		else if (level==3)
		{
			if (levelChange3==1)
			{
				...
				if (delay==100) 
				{
					...
					dx=8; range=6;
					maxlife=15; gld=25; grn=15; red=3;
...
void iKeyboard(unsigned char key)
{
	...
	if(gameStart==1)
	{
		...
		{
			iPauseTimer(0);
			printf("So, that's how they feel! It's like being the ");
			gets(cheat);
			if (strcmp(cheat,"batman")==0)
			{
				life=maxlife;
				printf("You cannot break me.\n");
				iResumeTimer(0);
			}
			...
			else if(strcmp(cheat,"bane")==0)
			{
				life=2;
				printf("I was born in the darkness.\n");
				iResumeTimer(0);
			}
